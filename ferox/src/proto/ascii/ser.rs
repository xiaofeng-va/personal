...
impl<'a> Serializer for &'a mut AsciiSerializer {
    ...
    fn serialize_unit_variant(
        ...
        info!("Serializing unit variant: {}", variant);
...
impl<'a> Serialize for FeroxRequest<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        match self {
            &FeroxRequest::Version => serializer.serialize_str("ver"),
            FeroxRequest::Failure { error } => todo!(),
            FeroxRequest::Another(another) => {
                serializer.serialize_str("another")?;
                another.serialize(serializer)
            }
            FeroxRequest::SmcForward { data } => todo!(),
        }
    }
}
